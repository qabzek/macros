...
int main(int argc, char* argv[]) {
  ...
  TH1F *hVtxZ = new TH1F("hVtxZ","hVtxZ",
			 140, -70., 70.);
  TH2F *hRefMultCorr9Cent = new TH2F("hRefMultCorr9Cent",";Centrality, %;<refmultCorrUtil>",9,0,9,1000,0,1000);
  ...
  for(Long64_t iEvent=0; iEvent<events2read; iEvent++) {
    ...
    refmultCorrUtil->initEvent(event->refMult(),pVtx.Z(),event->ZDCx());
    if (refmultCorrUtil->getCentralityBin9() < 0) continue;
    Int_t icent = refmultCorrUtil->getCentralityBin9();
    hRefMultCorr9Cent ->Fill(icent, refmultCorrUtil->getRefMultCorr());
